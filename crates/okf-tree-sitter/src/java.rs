...
/// `class_declaration`/`interface_declaration`/`enum_declaration` have no
/// `body` field named as such on all variants consistently — truncating
/// at the first `{` gives just the declaration line either way.
