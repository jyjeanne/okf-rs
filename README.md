...
`okf-mcp --benchmark-tool-selection` answers a different question — not "is registering this server worth it," but "does a model pick the right `relation` inside the consolidated `graph` tool as reliably as it used to pick the right tool name," before that 18→6 consolidation happened. Opt-in and never run in CI (it needs a real model call): point it at any OpenAI-compatible `/chat/completions` endpoint with function-calling support via `OKF_BENCHMARK_MODEL_BASE_URL`/`OKF_BENCHMARK_MODEL`(/`OKF_BENCHMARK_MODEL_API_KEY`) — deliberately separate variables from `search_semantic`'s `OKF_ENRICH_*`, so the benchmarked model and the enrichment model can differ — and it runs a fixed 14-question set against both the live `graph` tool and 13 reconstructed pre-consolidation `graph_*` tools on a known-correct fixture bundle, reporting tool-selection accuracy, final-answer accuracy, tokens, and latency for each design. A wrong selection is reported as one of three different-cost outcomes, not one undifferentiated "wrong": `[LOUD-FAIL]` (no tool matched, or a schema/argument the model or tool itself rejected — visible and retryable), `[DETECTABLE-WRONG]` (the wrong tool/relation, but its response is itself one of `okf-query`'s own empty/negative "nothing found" results — a signal a downstream consumer could act on without knowing the right answer), or `[SILENT-WRONG]` (a well-formed call to the wrong tool/relation whose response is real, populated data — the expensive one, since nothing about the response itself signals a retry is needed). It also reports `requests_per_answered_question` — `1 / final-answer accuracy`, cost expressed in requests rather than tokens, so it stays comparable regardless of session length. See [`docs/improvement-plan-provenance-diff.md`](docs/improvement-plan-provenance-diff.md#6-phase-e--specialized-vs-consolidated-mcp-tool-selection-benchmark--shipped) for the full design, its [Phase G](docs/improvement-plan-provenance-diff.md#11-phase-g--benchmark-scoring-and-ci-signal-follow-up-medium-review-august-2026--shipped) for the failure-mode/requests-per-answered-question follow-up, and [`ROADMAP.md`](ROADMAP.md#improvement-plan--provenance-depth-graph-diff--mcp-tool-selection) for how it was verified.
