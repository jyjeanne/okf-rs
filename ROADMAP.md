...
- [ ] Extended language coverage: ~~Java~~, Kotlin, C#, C/C++, PHP, Swift — **Java** shipped (classes, interfaces, enums, methods/constructors, `public`-modifier visibility, and a single `method_invocation` query pattern that captures bare/`this.`/object-receiver/static calls uniformly); Kotlin, C#, C/C++, PHP, and Swift remain
...
**Known limitations:**
- Public/private (`is_public`) detection is exact for Rust (`pub` modifier), Go (capitalization, the language's real rule), and Java (`public` modifier), but a naming-convention heuristic (leading underscore) for Python/JavaScript/TypeScript, pending real `export`/access-modifier tracking.
- Java's per-file `Module` concept follows the same convention as every other language (one module per source file), not Java's actual package-spans-multiple-files model; a package declaration doesn't yet merge same-package files into one module concept.
