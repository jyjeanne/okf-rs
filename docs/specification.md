...
Every `relationships.<kind>` entry `okf-rs` writes also carries edge-level provenance and confidence — e.g. `{target: functions/decode_jwt, resolved_by: tree-sitter, confidence: exact}` instead of a bare target id. `resolved_by` names what produced the edge: `tree-sitter` for Tree-sitter's own unambiguous, project-wide name match, or the binary name of the language server that resolved it (`rust-analyzer`, `pyright-langserver`, ...) when `generate --lsp` disambiguated an otherwise-ambiguous call. `confidence` is `exact` (Tree-sitter, syntax-derived) or `semantic` (LSP-resolved) accordingly — the same distinction an earlier draft of this feature considered a separate `kind: syntax`/`kind: semantic` field for, folded into one field since `confidence` already carries it. `okf_parser::read_bundle` also accepts a bare target-id string in this position, so a bundle written before this field existed (or a hand-edited entry kept simple) stays fully valid without regenerating.
...
- Frontmatter `relationships` targets (`calls`, `called_by`, `imports`, `member_of`, ...) resolve to concepts that exist in the bundle (external targets exempted) — whether each target is written as a bare id or the current `{target, resolved_by, confidence}` mapping
- A populated `confidence` that isn't `exact`/`semantic`, or an explicitly empty `resolved_by`, on a `relationships` entry is an error; a missing `resolved_by`/`confidence` isn't flagged, since it just falls back to the same default a bare-string entry gets
...
Validation is deterministic and fully offline, and is designed to run in CI (e.g. `okf-rs validate --ci`) to fail a pipeline on a broken bundle. "Stale" here means broken by `validate`'s own checks (e.g. a dangling relationship target left behind by hand-editing); it does *not* mean "out of sync with the current source tree" — `validate` never re-reads source, so a bundle can be perfectly valid by every check above and still not reflect what the project's source currently looks like. `okf-rs generate --check-fresh` is the check for that: it re-analyzes the project and diffs the result against the bundle already on disk, reporting exactly which files are stale (or that everything's up to date) without writing anything itself — the tool for a project that commits its bundle to git and wants CI to catch a contributor who changed source without regenerating.

**Caveat: this determinism guarantee covers `validate` itself and the default, tree-sitter-only `generate` path — not a bundle produced with `generate --lsp`.** `--lsp` resolves ambiguous calls by querying a real language server (`rust-analyzer`, `pyright-langserver`, ...), whose answer can depend on that server's index state in the environment it ran in, not on source text alone. A bundle generated with `--lsp` on one machine and regenerated with `generate --no-cache` on a different (e.g. colder) CI runner can legitimately disagree on some `Calls` edges with no underlying source change — and `validate --ci` cannot yet distinguish that from a real regression (see [`ROADMAP.md`'s Phase 2 known limitations](../ROADMAP.md#phase-2--depth--integration-) for the concrete scenario, and the "Strengthen deterministic-build guarantees" item in the [AI-native platform maturity improvement plan](../ROADMAP.md#improvement-plan--ai-native-platform-maturity-community-feedback) for `generate --check-determinism`, the tool now shipped to actually check for this within one environment). Edge provenance (also shipped — see the "Record edge provenance" item in that same plan) records which resolver produced each edge, so a future diff could in principle tell "the edge set changed" apart from "the same edges are recorded, with a different resolver" — but that distinction isn't wired into `validate --ci`/`diff` yet; today's tooling still reports any content difference, including a `resolved_by` change alone, as one undifferentiated mismatch. Until it is, generate the bundle CI validates with the same `--lsp`/no-`--lsp` flags, in the same kind of environment, every time.
